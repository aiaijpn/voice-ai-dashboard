...
`.trim();
    ...
    log(`🤖 [${rid}] (service/index) calling OpenAI...`);
...
`.trim();
    ...
    log(`✅ [${rid}] (service/index) OpenAI response received`);
    ...
    try {
      if (appendUsageRow) {
        ...
        log(`💰 [${rid}] (service/index) Usage saved`);
      }
    } catch (e) {
    ...
    log(
      `💬 [${rid}] (service/index) replyText=${String(replyText)
        .slice(0, 200)
        .replace(/\n/g, "\\n")}`
    );
    ...
    try {
      if (parsed) {
        ...
        log(`📄 [${rid}] (service/index) VoiceLog saved`);
      }
    } catch (e) {
